...
export async function generateIndexHTML(docsDir, currentDate) {
  let files = [];
  try {
    const entries = await fs.readdir(docsDir);
    files = entries
      .filter(f => f.startsWith('road-rage-') && f.endsWith('.html'))
      .sort()
      .reverse();
  } catch {}

  const reportItems = files.map(f => {
    const dateStr = f.replace('road-rage-', '').replace('.html', '');
...
<title>路怒研究日報 | Road Rage Research Daily</title>
<meta name="description" content="路怒症、攻擊性駕駛、駕駛憤怒研究文獻每日報告">
...
<div class="container">
  <header>
    ...
    <p class="desc">每日自動從 PubMed 抓取路怒症、攻擊性駕駛、駕駛憤怒相關的最新研究文獻，由 AI 分析彙整為中文日報。</p>
  </header>

  <div class="reports-section">
    <h2 class="section-title">📅 歷史日報</h2>
    ${files.length > 0 ? reportItems : '<p class="no-reports">尚無日報，請等待第一次自動執行。</p>'}
  </div>
